...
%.md: %.typ
	...
	echo '    <picture>' >> $@
	file="$<"; \
	echo "      <source media=\"(prefers-color-scheme: dark)\" srcset=\"$(GITHUB_PAGES_URL)/typ-build/$${file%.*}-dark.typ.svg\">" >> $@; \
	echo "      <source media=\"(prefers-color-scheme: light)\" srcset=\"$(GITHUB_PAGES_URL)/typ-build/$${file%.*}-light.typ.svg\">" >> $@; \
