...
%.md: %.typ
	...
	echo '    <picture>' >> $@
	file="$<"; \
	echo "      <source media=\"(prefers-color-scheme: dark)\" srcset=\"/typ-build/$${file%.*}-dark.typ.svg\">" >> $@; \
	echo "      <source media=\"(prefers-color-scheme: light)\" srcset=\"/typ-build/$${file%.*}-light.typ.svg\">" >> $@; \
