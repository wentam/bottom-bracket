...
Try to re-read this occassionaly when working on arrp so we remember important things.
...
* There should be no command line arguments to the arrp binary whatsoever to avoid the platform-dependent and implementation-dependent nature of that, and the fact that we want everything to be standardized. See some of the points below on how this can be avoided. Also see tag [A] in this file.
* important: arrp ALWAYS only evaluates one form from the input, there is no implicit top-level list or anything like that. arrp will provide a way to wrap the input in some top-level form, so if you want you can wrap in a list or in a macro entering your language of choice. See point below about how to wrap without command line arguments in a really flexible way.
...
* To wrap your input in something, just use (include "file.arrp") in your input to arrp to template it. Example: echo '(with-popped-printer-macros (include "my-real-code.arrp")) | arrp'. This might be most ergonomic if 'include' is a builtin macro, though think about if there's a good way to avoid this.
* [A] If you want to define a "compile-time variable" like -D to define something, do that by wrapping at CLI time like above with a macro (not builtin) that defines them. echo '(with-macros (include "my-language.arrp") (with-my-language (with-defined (LINUX X11 WAYLAND) (include "my-code.arrp"))))' | arrp. This approach is more flexible as the "CLI" arguments that are supported are not hardcoded into arrp, and the arguments aren't implementation-defined.
...
* We probably need a builtin macro that expands into the platform of the arrp binary, such that for macros your language macro knows what type of machine code to expand into. Maybe: (arrp-platform) expands into "x86_64-linux" (arrp-cpu) expands into "x86_64"
...
* Typical programming language compilation command example: $ echo '(with-macros (include "my-language.arrp") (with-my-language (with-defined ((OS LINUX) X11 WAYLAND) (include "my-code.arrp"))))' | arrp
                                  Or with the 'with' macro: $ echo '(with ((macros-from "my-language.arrp")
                                                                           my-language
                                                                           (defined ((OS LINUX) X11 WAYLAND)))
                                                                      (include "my-code.arrp"))' | arrp
* Or maybe just have a 'nest' macro:
  (nest (with-macros (include "my-language.arrp"))
        (with-my-language)
        (with-defined (stuff))
        (include "my-code.arrp"))

  Expands into:
    (with-macros (include "my-language.arrp")
      (with-my-language
        (with-defined (stuff)
          (include "my-code.arrp"))))
...
* When we say (with-macros (include "foo.arrp") (with-macros (include "bar.arrp") my-code)), the macros from foo.arrp would be available inside bar.arrp. This isn't *that* wierd, as it behaves the same way in C with #include, but we might want to provide a mechanism that locally reverts to just builtin macros, like (with-only-builtins foo). with-macros - or another macro with a similar name, could automatically use with-only-builtins.
* Note that you could build an interpreted language in arrp by using macros for their side effects instead of what they expand into. Could implement python inside arrp like this.
...
* include probably needs to be a builtin macro. Yes, you could implement it in arrp, but there would be no way to include the include macro as a library.
* idea: markup language like markdown/latex implemented via arrp macros (expands into some renderable format?)
* should arrp understand that there are different platforms for machine code? When defining a macro, you could define ((x86_64 "machine_code") (risc-v "machine_code")) to support different platforms for macro execution. This could also be left up to the user to resolve by having the user access an (arrp-platform) macro and switching themselves. This may make sense to occur in arrp though, because arrp's core functionality has to do with the execution of machine code. Building this into arrp would avoid undefined behavior if you accidentally choose to execute machine code from one platform on another, and building this into arrp would be very very simple (the x86_64 implementation of arrp just needs to always look up the x86_64 machine code.)
...
Currently strongly leaning towards resolving it within arrp. Avoids undefined behavior, can easily emit a warning if you define a macro without an implementation for the arrp execution platform, can easily emit an error when you try to call a macro when there isn't an implementation for the arrp execution platform, more ergonomic, doesn't really add complexity to arrp (but would be a bit complex to do it inside the arrp language), completely thetical to arrp's role as managing the execution of machine-language macros. Implementing this yourself inside the arrp language would *not* allow you to produce a nice error upon macroexpansion, so I think this is a must.

Question is: is this by cpu arch? x86_64, arm? or By platform: x86_64-linux, arm-windows. My current intuition says platform: I think you want a separate entry to occur per implementation required - which itself is a good way to define "platform" within arrp. Windows and linux have different "system" apis, linux with syscalls and windows with win32, and this demands separate implementations.

This does result in a combinatorial explosion of assemblers required to be implemented in machine language inside the arrp language, but I think that's just reality.

Note: to avoid actually running an assembler/compiler for all supported platforms when defining a macro, you could have your IR assembler/compiler accept an instruction to only produce a specified platform - then pass it (arrp-platform).

Another interesting thing to note is that this would allow arrp implementations to do some weird stuff like support execution of macros written for another platform through virtualization. This is actually a good argument to NOT have your IR assembler expand into only one but always provide all options. This is actually really interesting because it doesn't actually assume what our host platform is, we're just listing the ways we know how to accomplish the task and letting the arrp implementation choose whichever it knows how to execute best. It's also probably not that big of a deal to have your high-level language expand into all, because this portability is implemented at the IR level which should be a case of quick assembly for all.

If needing to assemble and optimize the IR 20 times is too expensive, arrp implementations could also provide a list of the macro execution platforms it supports sorted by priority, and the IR could limit it's expansions. Probably also providing stubs for other supported platforms so you can still see a list of supported platforms inside arrp.

Another perspective: this is actually a little bit weird, because you only ever need one platform at a given time. (with-macros) could also always only accept the correct machine code for the arrp execution platform, and you could also provide a (select-platform) macro that expands into the relevant platform for convenience:
...
* Due to the above, we could support a lot of platforms easily by for example using box64 in an ARM implementation of arrp to support macroexpansion.

* To bootstrap arrp, we could implement it once in as simple of an assembly language as possible - like RISC-V - then demand that those who wish to bootstrap must run a RISC-V virtual machine. If you keep it to a minimal portion of the risc-V instruction set, said VM could be very simple. We could even write some VMs.
* Idea: support execution of macros written for a different platform/arch through virtualization in more advanced implementations of arrp.
* Helpful line for documentation purposes: "arrp exposes all levels of abstraction for you to see and interact with, including machine language."
* Once we have a high-level language, a video walking through the abstractions bottom-up would be a great way to demonstrate what arrp is about.
* We probably want to implement macros in the arrp language (not builtin) for elf executables (not just relocatable object files), as well as implement a linker directly in macros. That way, you can actually specify your entire build process directly inside arrp.

(link-elf-executable (elf64-relocatable foo) (include "somefile_with_elf64-relocatable.arrp"))

Would expand into an elf executable.

No makefiles required! It's all arrp!

Worth thinking about how we would implement incremental builds with this.
* If we're writing our own assembler in machine language, could we share this assembler implementation for fully-verifiable bootstrapping? Could we writing it in a simple file format like stage0 "hex", then for the in-arrp implementation include and parse it into arrp structures?
* I don't like that byte_buffer stuff needs to be part of the public interface with the current
  design, and used when implementing macros. I want byte_buffer to be an implementation detail and I want as minimal of a public interface as possible. I think we need to rethink this interface.
    * macro(*input_structure, *cleanup_out) -> absolute output pointer or NULL for nothing. Macro writes a pointer to a cleanup function to *cleanup_out such as free() or byte_buffer_free() if it wants arrp to free it's data when it's done with it.
    ...
    * counter-argument: the byte_buffer functions like byte_buffer_push_barray are highly convienient for constructing output, and if we're going to provide this as public anyway we may as well use it as the default interface.
        * but of course, this could all still be implemented inside arrp-land.
...
* I'm starting to think that there should be no self-hosted implementation. I want to encourage everyone to go through fully verifiable bootstrapping paths, and arrp is so simple the assembly implementations can be well-optimized. Either way we need an assembly implementation per-platform to bootstrap.
    * counter-argument: not every platform needs a bootstrap implementation necessarily, as arrp can be cross-compiled from another machine or through virtualization as a bootstrap process. A self-hosted implementation could then cover a wider range of platforms
* We probably want to give the user access to the macro stack functions in user-defined macros (by providing macros like (arrp/builtin-func-addr/macro_stack_push)), as well as give them access to generally anything else we do with builtin macros that we're willing to standardize as a public interface
...
* We probably with a nice way to do (arrp/with-data ((my-str "foobar") (my-other-data "\xFF\xFF"))). Perhaps using what we currently call the "macro stack".
    * Make it store arrp structures, not just barrays. You should be able to say (arrp/with-data ((my-data (foo bar baz)))).
...
* We need to think about how build-time macro libraries could be distributed.
    * They could potentially be distributed as .so files that contain a function you can call to push all of it's macros - or simple all of it's macros listed as data with a symbol to reference them (a "global"). arrp could have a standard way of loading these with (load-so-macros).
        * This is fancy because you could include both runtime library components and buildtime components in a single .so file.
    * They could be distributed as raw .arrp files of source, but that would require building all build-time dependencies from source everytime (slow)
    * They could be distributed as raw arrp structures fully macroexpanded (compiled) in binary format, maybe call it something like file.arrpb64.
...
* We want to make sure we expose the relevant builtins in such a way that an arrp user could create their own (macro,data,etc) stacks
...
* If we had a parray-cat builtin, you could include multiple different .arrp files with macro lists and use them all in one with-macros call.
* it might be useful for barray-cat to have an (arrp/barray-cat/offset-labels 5) that counts as an offset as that element is hit.
...
* If you just build a language with macros that implements your language's behavior entirely at arrp expansion time, you have a JIT-compiled language
    * Also worth observing that at build-time in a compiled language, you're still working with a JIT language.
    * This means you could build a javascript JIT implementation inside arrp (I wonder about building a JS engine, maybe use it w/ ladybird browser).
* In fact, arrp can cleanly serve as all 3, making this a truely universal language-building tool:
...
* Idea: re-implement nasm inside arrp. Now we're self-hosting without actually having a separate impl.
* Pretty early on, we probably want to implement arrp and some of our abstractions on a few different platforms to make sure we're doing portability right:
...
* Right now, builtin function addresses are provided through macros like (arrp/builtin-func-addr/byte-buffer-new). This is pretty simply when the arrp implementation only supports one platform, but if the arrp implementation supports multiple,
...
* What if a macro can choose to *not* expand?
    ...
    * builtin macro using this: (arrp/hold foo) expands right back into (arrp/hold foo) returning -2 to prevent recursion.
